...
public class GAEAsyncTwitterTestBase extends TestCase {
    public GAEAsyncTwitterTestBase(String name) {
        super(name);
    }

    protected GAEAsyncTwitter twitter1, twitter2, twitter3,
    ...
    protected void setUp() throws Exception {
        super.setUp();
        InputStream is = GAEAsyncTwitterTestBase.class.getResourceAsStream("/test.properties");
        p.load(is);
        is.close();
...
//        id1id = Integer.valueOf(p.getProperty("id1id"));
        ...
        twitter1 = new GAEAsyncTwitterFactory(conf1).getInstance();

        twitter2 = new GAEAsyncTwitterFactory(conf2).getInstance();

        twitter3 = new GAEAsyncTwitterFactory(conf3).getInstance();

        twitterAPIBestFriend1 = new GAEAsyncTwitterFactory(bestFriend1Conf).getInstance();

        twitterAPIBestFriend2 = new GAEAsyncTwitterFactory(bestFriend2Conf).getInstance();

        unauthenticated = new GAEAsyncTwitterFactory().getInstance();
